...
async function handleErrorResponse(state: GraphState, llmClient: OpenRouterService): Promise<Partial<GraphState>> {
  ...
  if (error) {
    const messageError = `An error ocurred: ${error}`
    return {
      messages: [new AIMessage(messageError)],
      error,
      answer: messageError,
      followUpQuestions: []
    }
...
async function handleSuccessResponse(state: GraphState, llmClient: OpenRouterService): Promise<Partial<GraphState>> {
  ...
  const { data, error } = await llmClient.generateStructured(_userPrompt, systemPrompt, AnalyticalResponseSchema)
