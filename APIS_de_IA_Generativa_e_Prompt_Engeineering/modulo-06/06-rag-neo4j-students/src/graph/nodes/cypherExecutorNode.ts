...
async function executeQuery(query: string, neo4Service: Neo4jService) {
  try {

    const isValid = await neo4Service.validateQuery(query)
    if (!isValid) {
      return {
        results: null,
        error: 'Query validation failed - syntax or struture error'
      }
    }

    let results = await neo4Service.query(query)
    if (!results.length) {
      return {
        results: [],
    ...
    console.log(`✅ Retrived ${results.length} result(s)`)
...
export function createCypherExecutorNode(neo4jService: Neo4jService) {
  ...
  return async (state: GraphState): Promise<Partial<GraphState>> => {
    try {
      ...
      if (state.isMultiStep && state.subQuestions?.length && state.currentStep !== undefined) {
        const multiStepsState = handleMultiStepProgression(state, results!)
        return {
          ...multiStepsState
        }
      }
