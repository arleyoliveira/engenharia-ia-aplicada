...
export async function blockedNode(state: GraphState): Promise<Partial<GraphState>> {
  ...
  const analysis = guardrailCheck ? `**Analysis:** ${guardrailCheck.analysis}` : '';

  const permissions = state.user.permissions?.join(', ') || 'No permissions available'; 

  const template = PromptTemplate.fromTemplate(prompts.blocked);

  const blockedMessage = await template.format({
    REASON: state.guardrailCheck?.reason || 'Security check failed',
    ANALYSIS: analysis,
